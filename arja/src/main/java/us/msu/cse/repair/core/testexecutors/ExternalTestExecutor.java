...
public class ExternalTestExecutor implements ITestExecutor {
	...
	public boolean runTests() throws IOException, InterruptedException {
		...
		params.add(jvmPath);
		params.add("-cp");
		System.out.println("test exec"+jvmPath);
		String cpStr = "";
		...
		System.out.println("test exec 2"+cpStr);
		if (dependences != null) {
			for (String dp : dependences)
				cpStr += (File.pathSeparator + dp);
		}
		System.out.println("test exec 3 "+cpStr);
		params.add(cpStr);
		...
		System.out.println("test exec 5 "+params.toString());
		...
		System.out.println("test exec 7 "+builder.toString());
		...
		System.out.println("test exec 8 "+streamReaderThread.isStreamExceptional());
		System.out.println("test exec 9 "+streamReaderThread.getOutput());
